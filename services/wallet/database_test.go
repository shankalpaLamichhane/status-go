...
func TestDBProcessBlocks(t *testing.T) {
	...
	db.SaveTranfers(address, transfers, []*big.Int{big.NewInt(1), big.NewInt(2)})
}
...
func TestDBProcessTransfer(t *testing.T) {
	...
	db.ProcessBlocks(common.Address{1}, big.NewInt(1), big.NewInt(1), []*DBHeader{header})
...
func TestDBReorgTransfers(t *testing.T) {
	...
	db.ProcessBlocks(original.Address, original.Number, original.Number, []*DBHeader{original})
	...
	db.ProcessBlocks(replaced.Address, replaced.Number, replaced.Number, []*DBHeader{replaced})
	require.NoError(t, db.ProcessTranfers([]Transfer{
...
func TestDBGetTransfersFromBlock(t *testing.T) {
	...
	db.ProcessBlocks(headers[0].Address, headers[0].Number, headers[len(headers)-1].Number, headers)
	require.NoError(t, db.ProcessTranfers(transfers, []*DBHeader{}))
